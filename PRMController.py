...
class PRMController:
    ...
    def genCoords(self,x_range,y_range,nbrs,pointPlot):
        self.collisionFreePoints = np.array([])
        generatednbrs = 0
        while generatednbrs < nbrs:
            x_list = x_range*self.rng.random(1)
            y_list = y_range*self.rng.random(1)
            if not(self.env.check_collision(x_list[0],y_list[0])):
                if(self.collisionFreePoints.size == 0):
                    self.collisionFreePoints = np.array((x_list[0],y_list[0]))
                    generatednbrs += 1
                else:
